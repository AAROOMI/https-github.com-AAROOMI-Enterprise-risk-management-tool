...
import { DeleteIcon, MicrophoneIcon, GapIndicatorIcon, ShieldCheckIcon } from './icons';

interface GapAssessmentTableProps {
  risks: Risk[];
...
const GapAssessmentTable: React.FC<GapAssessmentTableProps> = ({ risks, onSelectRisk, onDeleteRisk, companyName, assessorName, onStartAssessment, isAssistantActive }) => {
  ...
  const tableHeaders = [
    { name: "Risk ID", print: true },
    { name: "Risk Title", print: true },
    { name: "Mitigating Actions", print: true },
    { name: "Likelihood", print: true },
    { name: "Impact", print: true },
    { name: "Risk Level", print: true },
    { name: "Control Effectiveness", print: true },
    { name: "Gap Description", print: true },
    { name: "Remediation Plan", print: true },
    { name: "Linked Controls", print: false }, // New column
    { name: "Action", print: false },
  ];
  ...
  return (
    <div className="px-4 pb-8">
      <div className="pt-2 pb-4 md:flex md:items-center md:justify-between print:hidden">
        <div>
          <h2 className="text-2xl font-bold text-cyan-300">Gap Assessment</h2>
          <p className="text-gray-400 text-sm mt-1">Review risks based on control effectiveness and define remediation plans.</p>
        </div>
        <div className="mt-4 md:mt-0">
          ...
          >
            <MicrophoneIcon /> Start Gap Assessment
          </button>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <div className="hidden print:block mb-4 text-black">
          <h2 className="text-xl font-bold">Gap Assessment - {companyName}</h2>
          ...
          <thead className="bg-slate-800/60 print:bg-gray-200">
            <tr>
              {tableHeaders.map((header) => (
                <th key={header.name} className={`p-2 border border-slate-700 font-semibold text-left print:border-gray-400 ${!header.print ? 'print:hidden' : ''}`}>
                  {header.name}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {filteredRisks.map((risk) => (
              <tr key={risk.id} onClick={() => onSelectRisk(risk.id)} className="hover:bg-slate-700/50 transition-colors duration-200 cursor-pointer">
                <td className="p-2 border border-slate-700 print:border-gray-400 align-top">
                  {risk.RiskID}
                </td>
                <td className="p-2 border border-slate-700 print:border-gray-400 align-top">
                ...
                <td className="p-2 border border-slate-700 print:border-gray-400 align-top whitespace-pre-wrap">
                ...
                <td className="p-2 border border-slate-700 text-center print:border-gray-400 align-top print:hidden">
...
export default GapAssessmentTable;
